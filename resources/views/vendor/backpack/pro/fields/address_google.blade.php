...
{{-- If a field type is shown multiple times on a form, the CSS and JS will only be loaded once --}}
    ...
    @bassetBlock('backpack/pro/fields/address-google-field.css')
        <style>
    ...
    @push('crud_fields_scripts')
        ...
        @loadOnce('google_places_api_script')
            <script async defer src="https://maps.googleapis.com/maps/api/js?v=3&key={{ $field['api_key'] ?? config('services.google_places.key') }}&loading=async&libraries=places,marker&callback=initGoogleAddressAutocomplete&language={{$field_language}}"></script>
        @endLoadOnce
    @endpush
